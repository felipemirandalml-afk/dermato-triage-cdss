...
export const ResultsPanel = () => {
  ...
  const triageConfig = {
    P1: { title: t('engine.priority.P1'), color: 'bg-triage-p1 text-white border-triage-p1', bg: 'bg-triage-p1-bg', icon: 'P1', action: t('engine.conduct.urgent') },
    P2: { title: t('engine.priority.P2'), color: 'bg-triage-p2 text-white border-triage-p2', bg: 'bg-triage-p2-bg', icon: 'P2', action: t('engine.conduct.priority') },
    P3: { title: t('engine.priority.P3'), color: 'bg-triage-p3 text-white border-triage-p3', bg: 'bg-triage-p3-bg', icon: 'P3', action: t('engine.conduct.standard') }
  };
    ...
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
      <div className={`p-6 rounded-2xl border-2 flex items-center justify-between ${config.color} shadow-lg`}>
        <div className="flex items-center gap-4">
          <span className="text-4xl font-black">{config.icon}</span>
          <div>
            <div className="flex gap-2 text-xs font-bold opacity-90 mb-1">
              <span className="bg-white/20 px-2 py-0.5 rounded-md">Edad {formData.age || '?'}</span>
              <span className="bg-white/20 px-2 py-0.5 rounded-md capitalize">{sexLabel}</span>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">{t('ui.first_suspicion', { defaultValue: '1ra Sospecha Sindromica' })}</span>
            <div className="flex items-end gap-3 mb-4">
              <h3 className="text-2xl font-black text-slate-800 capitalize">{primary_syndrome ? primary_syndrome.replace(/_/g, ' ') : t('ui.indet_group', { defaultValue: 'Agrupacion Indeterminada' })}</h3>
            ...
            {differential_ranking && differential_ranking.length > 0 && (
              <div className="mt-6 pt-4 border-t border-slate-200">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 block">Diagnosticos Diferenciales (Top 3)</span>
                  ...
                  {differential_ranking.slice(0, 3).map((diff, index) => (
                    <div key={index} className="flex justify-between items-center p-2 hover:bg-white rounded-lg transition-colors">
                      <span className="text-sm font-semibold text-slate-700 capitalize">
                        {index + 1}. {diff.disease_name ? diff.disease_name.replace(/_/g, ' ') : 'Agrupacion Desconocida'}
                      </span>
          ...
          <div className="bg-white border text-slate-800 border-slate-200 rounded-2xl p-5 shadow-sm relative overflow-hidden">
            ...
            {triggered_rules && triggered_rules.length > 0 ? (
              <div className="space-y-3">
                {triggered_rules.map((rule, idx) => (
                  <div key={idx} className="flex items-start gap-3 p-3 bg-red-50/50 border border-red-100 rounded-xl text-sm">
                    ...
                    <div>
                      <strong className="text-red-800 block text-xs tracking-wide uppercase mb-0.5">Escudo Heuristico Activado</strong>
              ...
              <div className="p-4 bg-slate-50 rounded-xl text-sm text-slate-500 italic text-center">
                {t('ui.no_emergency_rules', { defaultValue: 'Ninguna regla de emergencia vital disparada. El triaje procede segun estadistica basal.' })}
              </div>
            )}
